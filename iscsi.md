#!/bin/bash
###################################################################
# Linux iSCSI 구성 스크립트 (Target + Initiator 통합)
# 작성자: (작성자 이름)
# 목적: Target 서버와 Initiator 서버에서 iSCSI 디스크 구성
# 주의: IP와 디스크 장치(/dev/sdb) 확인 후 사용
###################################################################
...
# 1.8 ACL 생성 → Initiator(IP 142, IQN) 접근 허용
...
# 2.7 부팅 시 자동 마운트 등록 (수동으로 /etc/fstab 편집 필요)
